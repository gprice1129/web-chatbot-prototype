...
End-to-end exercise of `DELETE /api/projects/:project_id`. Logs in as `testuser`, deletes the given project, and prints the `{ message }` response. Deleting a project removes only the `project_chats` link rows — its member chats survive.
